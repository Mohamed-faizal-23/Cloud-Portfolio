...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Mobile menu toggle + responsive behavior (class-based)
  ...
  function handleNavResize() {
    ...
    if (window.innerWidth >= 760) {
      // Desktop: ensure nav is visible and remove mobile-open state
      nav.classList.remove('open');
      nav.style.display = ''; // let CSS handle desktop layout
      if (mobileBtn) mobileBtn.setAttribute('aria-expanded', 'false');
    } else {
      // Mobile: hide nav by default if not open
      if (!nav.classList.contains('open')) {
        nav.style.display = ''; // stay controlled by CSS (.site-nav hidden)
        if (mobileBtn) mobileBtn.setAttribute('aria-expanded', 'false');
      } else {
        // if open keep it visible (class handles display)
        nav.style.display = '';
  ...
  if (mobileBtn && nav) {
    mobileBtn.addEventListener('click', () => {
      // Only toggle on mobile widths; on desktop we do nothing
      if (window.innerWidth < 760) {
    ...
    // close nav when a link is clicked on mobile, so user can tap again
  ...
  const projectsData = {
    p1: {
      ...
      codeUrl: 'https://github.com/Mohamed-faizal-23/aws-static-site-hosting', // later: GitHub report/code link
      liveUrl: ''
    },
    p2: {
      badge: 'Cloud Project',
      ...
      codeUrl: 'https://github.com/Mohamed-faizal-23/aws-secure-vpc-architecture', // later: GitHub report/code link
      liveUrl: ''
    },
    p3: {
      badge: 'Cloud Project',
      ...
      codeUrl: 'https://github.com/Mohamed-faizal-23/aws-ci-cd-pipeline-deployment', // later: GitHub report/code link
      liveUrl: ''
    },
    p4: {
